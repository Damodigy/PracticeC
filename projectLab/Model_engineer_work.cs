...
using Model_engineer_work;

namespace Model_engineer_work
{
	class fuel
	{
		public string name;
...
public class Model_engineer_work
{
	public Model_engineer_work()
	{
private table_instruments instruments;
	...
	private void reload_data()
	{
		...
		query.CommandText = "SELECT COUNT(*) FROM `fuel_transactions` WHERE `fuel_volume` > 0";
		...
		while (result.Read())
		{
			...
			if (Convert.IsDBNull(result.GetValue(8)))
			{               //`trans`.`time_end`
				tmp.time_end = "";
			}
			else
